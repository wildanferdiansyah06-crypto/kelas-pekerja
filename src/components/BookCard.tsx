...
export default function BookCard({ book, index = 0, href, onClick }: BookCardProps) {
  ...
  // KALAU ADA onClick, render div (bukan Link)
  if (onClick) {
    return (
      <div 
        onClick={onClick} 
  ...
  // KALAU GAK ADA onClick, render Link (backward compatibility)
  const linkHref = href ?? `/buku/${book.slug ?? ""}`;
  
  return (
    <Link href={linkHref} prefetch className="group block w-full">
      <CardContent 
        book={book} 
        index={index} 
        isHovered={isHovered}
...
// Pisah content biar gak duplikat kode
