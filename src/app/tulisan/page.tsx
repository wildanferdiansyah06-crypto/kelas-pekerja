...
import { PenLine, Clock, ArrowRight, Filter, Sparkles, TrendingUp, ChevronRight } from "lucide-react";
    ...
    <main className={`min-h-screen ${colors.bg} ${colors.text} transition-colors duration-500`}>
      ...
      <div className="relative z-10">
        ...
        <motion.header
          ...
          initial="hidden"
          animate="show"
          className="pt-24 pb-12 px-4 sm:px-6"
        >
          <div className="max-w-5xl mx-auto text-center">
            <motion.div variants={fadeUp} className="flex items-center justify-center gap-3 mb-6">
              <div className="h-px w-12 bg-[#8b7355]"></div>
              <span className="text-[10px] tracking-[0.4em] uppercase text-[#8b7355] font-medium">
                Arsip Pikiran
              </span>
              <div className="h-px w-12 bg-[#8b7355]"></div>
            </motion.div>
            ...
            <motion.h1 
              variants={fadeUp}
              className={`font-serif text-4xl sm:text-5xl md:text-7xl mb-6 ${colors.heading} tracking-tight`}
            >
              <span className="block text-[#c7b299] text-xl sm:text-2xl md:text-3xl mb-3 italic font-light">
                ruang bagi
              </span>
              Tulisan
            </motion.h1>
            ...
            <motion.p 
              variants={fadeUp}
              className={`text-lg sm:text-xl ${colors.textMuted} leading-relaxed max-w-2xl mx-auto mb-8`}
            >
              Di antara deru waktu yang tak pernah berhenti, 
              ada saat-saat ketika kata-kata menjadi satu-satunya tempat perlindungan.
            </motion.p>
            ...
            <motion.div variants={fadeUp}>
              <Link
                ...
                className="inline-flex items-center gap-3 px-8 py-4 
                  ...
                  text-sm font-medium shadow-lg shadow-[#8b7355]/25"
              >
                <PenLine size={18} />
                Tulis Ceritamu
              </Link>
        ...
        <motion.section
          ...
          className={`py-6 border-y ${colors.border} overflow-hidden`}
        >
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            <div className="flex items-center gap-4 mb-4">
              <TrendingUp size={18} className="text-[#8b7355]" />
              <span className="text-xs tracking-widest uppercase text-[#8b7355] font-medium">
                Sedang Dibaca
              </span>
            </div>
            <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
              {trendingPosts.map((post: any) => (
                <Link
                  ...
                  className="flex-shrink-0 w-72 p-4 rounded-xl border border-[#8b7355]/20 
                    hover:border-[#8b7355] hover:bg-[#8b7355]/5 transition-all group"
                >
                  <span className="text-[10px] tracking-widest uppercase text-[#8b7355]">
                    {post.category}
                  </span>
                  <h3 className={`font-serif text-lg mt-2 ${colors.heading} group-hover:text-[#c7b299] line-clamp-2`}>
                    {post.title}
                  </h3>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
              ...
              <div className={`relative overflow-hidden rounded-3xl ${colors.featuredBg} border ${colors.border} p-6 sm:p-10`}>
                ...
                <div className="relative z-10 max-w-3xl">
                  ...
                  <div className="flex flex-wrap gap-2 mb-6">
                    <span className="px-3 py-1 bg-[#8b7355] text-white text-xs rounded-full">
                      {featuredPost.category}
                    </span>
                    <span className={`px-3 py-1 border ${colors.border} text-xs rounded-full ${colors.textMuted}`}>
                  ...
                  <h2 className={`font-serif text-3xl sm:text-4xl md:text-5xl mb-6 ${colors.heading} leading-tight`}>
                    {featuredPost.title}
                  </h2>

                  <p className={`text-lg ${colors.textMuted} leading-relaxed mb-8 max-w-2xl`}>
                    {featuredPost.excerpt}
                  </p>

                  <Link
                    href={`/tulisan/${featuredPost.slug}`}
                    className="inline-flex items-center gap-3 px-8 py-4 bg-[#8b7355] text-white rounded-full hover:bg-[#c7b299] transition-all text-sm font-medium"
                  >
                    Baca Pembukaan
                    <ArrowRight size={18} />
                  </Link>
          ...
          <div className={`flex items-baseline justify-between mb-8 border-b ${colors.border} pb-4`}>
            <h2 className={`font-serif text-2xl ${colors.heading} italic`}>
              {activeCategory === "Semua" ? "Semua Cerita" : activeCategory}
            </h2>
            <span className={`text-xs tracking-widest uppercase ${colors.textSubtle}`}>
              {filteredPosts.length} tulisan
            </span>
          </div>
              ...
              <motion.div variants={fadeUp} className={`col-span-2 text-center py-24 ${colors.textSubtle}`}>
                {regularPosts.length === 0 ? (
                  ...
                  <>
                    ...
                    <button onClick={() => setActiveCategory("Semua")} className="text-[#8b7355] hover:text-[#c7b299] text-sm">
              ...
              filteredPosts.map((post: any) => (
                ...
                >
                  <Link href={`/tulisan/${post.slug}`} className="block h-full">
                    <div className={`h-full p-6 sm:p-8 rounded-2xl border ${colors.border} 
                      hover:border-[#8b7355] transition-all duration-300 ${colors.cardBg}
                      ${hoveredPost === post.slug ? 'shadow-xl shadow-[#8b7355]/10' : ''}`}>
                      
                      {/* Meta */}
                      <div className="flex flex-wrap items-center gap-3 mb-4">
                        <span className="px-3 py-1 text-[10px] tracking-widest uppercase bg-[#8b7355]/10 text-[#8b7355] rounded-full">
                          {post.category}
                        </span>
                      ...
                      {/* Opening Hook */}
                      <p className={`text-sm italic mb-3 text-[#c7b299] line-clamp-2`}>
                        "{post.opening || post.hook}"
                      </p>

                      {/* Title */}
                      <h3 className={`font-serif text-xl sm:text-2xl mb-3 ${colors.heading} leading-tight
                        group-hover:text-[#c7b299] transition-colors`}>
                        {post.title}
                      </h3>

                      {/* Excerpt */}
                      <p className={`text-sm ${colors.textMuted} line-clamp-2 mb-4 leading-relaxed`}>
                        {post.excerpt}
                      </p>
        ...
        <section className={`px-4 sm:px-6 py-24 border-t ${colors.border}`}>
          <div className="max-w-3xl mx-auto text-center">
            <p className="font-serif text-3xl sm:text-4xl italic text-[#c7b299] mb-6">
