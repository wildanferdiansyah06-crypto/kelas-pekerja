...
export default async function BooksPage() {
  ...
  // Tambahkan slug ke setiap buku
  const booksWithSlugs = books.map((book) => ({
    ...book,
    slug: bookSlugMap[book.title] || book.title.toLowerCase().replace(/\s+/g, "-"),
  }));

  return (
    <main className="min-h-screen bg-[#faf8f5] dark:bg-[#1a1816] text-[#2b2b2b] dark:text-[#e8e0d5] transition-colors duration-700">
      {/* HEADER - Lebih besar dan bold */}
      ...
      {/* GRID BUKU - Lebih besar dan lebar */}
      <section className="px-6 pb-32">
        <div className="max-w-screen-2xl mx-auto">
          {booksWithSlugs.length > 0 ? (
            <>
              {/* Grid 2 kolom di desktop, lebih besar gapnya */}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-16 gap-y-20">
                {booksWithSlugs.map((book, index) => (
                  <BookCard 
                    key={book.id} 
                    book={book} 
                    index={index}
              ...
              <div className="mt-32 text-center">
                <div className="inline-flex items-center gap-3 px-6 py-3 rounded-full border border-[#2b2b2b]/10 dark:border-[#e8e0d5]/10">
                  <span className="w-2 h-2 rounded-full bg-[#2b2b2b]/40 dark:bg-[#e8e0d5]/40"></span>
                  <span className="text-sm opacity-50 tracking-wide">
                    Menampilkan {booksWithSlugs.length} dari {total} buku
                  </span>
                </div>
              </div>
            </>
