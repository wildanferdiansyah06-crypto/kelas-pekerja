...
export default function BookCard({ book, index = 0, href, onClick }: BookCardProps) {
  ...
  // KALAU ADA onClick, RENDER ARTICLE BIASA (BUKAN LINK)
  const CardWrapper = onClick ? 'div' : Link;
  const wrapperProps = onClick 
    ? { onClick, className: "group block w-full cursor-pointer" }
    : { href: linkHref, prefetch: true, className: "group block w-full" };
  ...
  return (
    <CardWrapper {...wrapperProps}>
      ...
      >
        <div
          className="relative aspect-[16/10] w-full mb-6 overflow-hidden rounded-xl
              ...
              <p className="text-white/90 text-sm font-medium tracking-wide flex items-center gap-2">
                ...
                <ArrowUpRight
                  ...
                  className={`transition-all duration-500 ${
                    isHovered ? "translate-x-1 -translate-y-1 opacity-100" : "opacity-70"
                  }`}
                />
              </p>
            </div>
        ...
        <div className="space-y-3">
          <h3 className="font-serif text-2xl md:text-[1.75rem] leading-[1.2] text-[#2b2b2b] dark:text-[#e8e0d5] opacity-90 group-hover:opacity-100 transition-all duration-300">
            {book.title}
          </h3>
