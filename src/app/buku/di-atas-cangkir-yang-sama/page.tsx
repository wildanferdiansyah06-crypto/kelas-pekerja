...
export default function CoffeeBookPage() {
  ...
  // Theme classes - refined for better contrast and readability
  ...
  return (
    <div className={`min-h-screen ${theme.bg} ${theme.text} transition-colors duration-500`}>
      ...
      {/* Fixed Header - Improved height and proportions */}
      <header className={`fixed top-0 left-0 right-0 z-50 ${theme.sidebar} border-b ${theme.border} backdrop-blur-xl bg-opacity-95`}>
        <div className="max-w-[1600px] mx-auto px-6 lg:px-8 h-20 flex items-center justify-between">
          <div className="flex items-center gap-6">
            <button 
              onClick={() => setSidebarOpen(!sidebarOpen)}
              className={`p-3 rounded-xl ${darkMode ? 'hover:bg-neutral-800' : 'hover:bg-stone-100'} transition-all duration-300 active:scale-95`}
            >
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="hidden sm:block">
                <h1 className={`font-bold text-lg ${theme.textHeading} tracking-tight`}>Coffee from Bean to Cup</h1>
                <p className={`text-sm ${theme.textMuted}`}>A Serious Guide</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className={`p-3 rounded-xl ${darkMode ? 'hover:bg-neutral-800' : 'hover:bg-stone-100'} transition-all duration-300 active:scale-95`}
            >
              {darkMode ? <Sun size={22} className={theme.accent} /> : <Moon size={22} className={theme.accent} />}
            </button>
          </div>
        </div>
      </header>

      {/* Sidebar Navigation - Improved width and positioning */}
      <AnimatePresence>
        {sidebarOpen && (
          <>
            <motion.div 
              ...
              className={`fixed inset-0 bg-black/60 z-40 backdrop-blur-sm ${isLargeScreen ? 'hidden' : ''}`}
            />
            <motion.aside
              initial={{ x: -320 }}
              animate={{ x: 0 }}
              ...
              className={`fixed left-0 top-20 bottom-0 w-72 lg:w-80 ${theme.sidebar} border-r ${theme.border} z-50 overflow-y-auto shadow-2xl`}
            >
              <div className="p-6">
                <h2 className={`text-xs font-bold uppercase tracking-widest ${theme.textMuted} mb-6`}>Daftar Isi</h2>
                <nav className="space-y-2">
                  {chapters.map((chapter) => (
                    <a
                      ...
                      onClick={() => !isLargeScreen && setSidebarOpen(false)}
                      className={`group block p-4 rounded-xl text-sm transition-all duration-300 ${
                        activeChapter === chapter.num 
                          ? `${theme.accentBg} ${theme.accent} font-semibold border ${theme.accentBorder} shadow-lg` 
                          : `${darkMode ? 'hover:bg-neutral-800/50' : 'hover:bg-stone-100'} ${theme.textMuted} hover:text-current`
                      }`}
                    >
                      <div className="flex items-start gap-4">
                        <span className={`text-sm font-mono mt-0.5 ${activeChapter === chapter.num ? theme.accent : theme.textMuted} group-hover:scale-110 transition-transform`}>
                          {String(chapter.num).padStart(2, '0')}
                        </span>
                        <div className="flex-1">
                          <p className={`font-medium text-base leading-tight ${activeChapter === chapter.num ? theme.textHeading : theme.text}`}>
                            {chapter.title}
                          </p>
                          <p className={`text-xs mt-1.5 ${theme.textMuted} leading-relaxed`}>{chapter.subtitle}</p>
                        </div>
                        {activeChapter === chapter.num && (
                          <ChevronRight size={16} className="mt-1 animate-pulse" />
                        )}
                      </div>
                    </a>
                  ))}
      ...
      {/* Main Content - Dramatically improved layout and proportions */}
      <main className={`pt-24 pb-32 transition-all duration-500 ${sidebarOpen && isLargeScreen ? 'ml-80' : ''}`}>
        <div className="max-w-5xl xl:max-w-6xl mx-auto px-6 sm:px-8 lg:px-12">
          
          {/* Book Header - Enhanced proportions and spacing */}
          <motion.div 
            initial="hidden"
            animate="visible"
            variants={fadeIn}
            className="mb-24 text-center pt-12"
          >
            <motion.div 
              whileHover={{ scale: 1.05, rotate: 5 }}
              className={`inline-flex items-center justify-center w-24 h-24 rounded-3xl ${theme.accentBg} border ${theme.accentBorder} mb-8 shadow-xl`}
            >
              <Coffee size={48} className={theme.accent} />
            </motion.div>
            
            <h1 className={`text-5xl sm:text-6xl lg:text-7xl font-bold ${theme.textHeading} mb-6 tracking-tight leading-tight`}>
              Coffee from Bean to Cup
            </h1>
            <p className={`text-2xl sm:text-3xl ${theme.textSubheading} italic mb-6 font-light`}>A Serious Guide</p>
            <p className={`text-lg ${theme.textMuted}`}>oleh Wildan Ferdiansyah</p>
            
            <motion.div 
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.3 }}
              className={`mt-12 p-8 rounded-2xl ${theme.highlight} border ${theme.border} max-w-3xl mx-auto shadow-lg`}
            >
          ...
          {/* Ringkasan Materi - Improved card layout */}
          <motion.section 
            initial="hidden"
            whileInView="visible"
            ...
            className="mb-28"
          >
            <h2 className={`text-3xl sm:text-4xl font-bold ${theme.textHeading} mb-10 pb-4 border-b-2 ${theme.border}`}>
              Ringkasan Materi Utama
            </h2>
            
            <div className="grid lg:grid-cols-3 gap-6">
              <motion.div variants={fadeIn} className={`p-8 rounded-2xl ${theme.card} border ${theme.border} shadow-lg hover:shadow-xl transition-shadow duration-300`}>
                <h3 className={`font-bold text-xl ${theme.accent} mb-5 flex items-center gap-3`}>
                  <span className={`w-10 h-10 rounded-full ${theme.accentBg} ${theme.accent} text-sm flex items-center justify-center font-bold`}>1</span>
                  Fondasi Biologis
                </h3>
              ...
              <motion.div variants={fadeIn} className={`p-8 rounded-2xl ${theme.card} border ${theme.border} shadow-lg hover:shadow-xl transition-shadow duration-300`}>
                <h3 className={`font-bold text-xl ${theme.accent} mb-5 flex items-center gap-3`}>
                  <span className={`w-10 h-10 rounded-full ${theme.accentBg} ${theme.accent} text-sm flex items-center justify-center font-bold`}>2</span>
              ...
              <motion.div variants={fadeIn} className={`p-8 rounded-2xl ${theme.card} border ${theme.border} shadow-lg hover:shadow-xl transition-shadow duration-300`}>
                <h3 className={`font-bold text-xl ${theme.accent} mb-5 flex items-center gap-3`}>
                  <span className={`w-10 h-10 rounded-full ${theme.accentBg} ${theme.accent} text-sm flex items-center justify-center font-bold`}>3</span>
                  Sains Ekstraksi & Air
                </h3>
                <ul className={`space-y-4 text-base ${theme.text}`}>
                  <li className="flex items-start gap-3">
          ...
          {/* Bab 1 - Enhanced typography and spacing */}
          <motion.section 
            id="bab-1"
            data-chapter={1}
            initial="hidden"
          ...
          >
            <div className={`flex flex-col sm:flex-row sm:items-center gap-6 mb-12 pb-6 border-b-2 ${theme.border}`}>
              <span className={`text-7xl sm:text-8xl font-black ${theme.accent} opacity-20 leading-none`}>01</span>
              <div className="flex-1">
                <h2 className={`text-4xl sm:text-5xl font-bold ${theme.textHeading} mb-3`}>Coffee Species</h2>
                <p className={`text-lg ${theme.textMuted}`}>Genus Coffea dan Posisi Botani Kopi</p>
              </div>
            </div>

            <div className="space-y-8">
              <div className={`p-8 rounded-2xl ${theme.highlight} border ${theme.border} shadow-lg`}>
                <h3 className={`font-bold text-2xl ${theme.textHeading} mb-5`}>1.1 Genus Coffea</h3>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className={`p-8 rounded-2xl ${theme.card} border ${theme.border} shadow-lg hover:shadow-xl transition-all duration-300`}>
                  <h4 className={`font-bold text-xl ${theme.accent} mb-5 flex items-center gap-2`}>
                    🌿 Arabica
                  </h4>
                  <ul className={`space-y-3 text-base ${theme.text}`}>
                    <li className="flex justify-between border-b border-dashed border-current/20 pb-2">
              ...
              <blockquote className={`pl-8 border-l-4 ${theme.accentBorder} italic ${theme.textSubheading} text-xl leading-relaxed`}>
          ...
          >
            <div className={`flex flex-col sm:flex-row sm:items-center gap-6 mb-12 pb-6 border-b-2 ${theme.border}`}>
              <span className={`text-7xl sm:text-8xl font-black ${theme.accent} opacity-20 leading-none`}>02</span>
              <div className="flex-1">
                <h2 className={`text-4xl sm:text-5xl font-bold ${theme.textHeading} mb-3`}>Coffee Processing</h2>
                <p className={`text-lg ${theme.textMuted}`}>Anatomi Buah dan Fermentasi</p>
              </div>
            </div>

            <div className="space-y-8">
              <p className={`${theme.text} leading-relaxed text-xl`}>
                <strong className={`text-2xl ${theme.textHeading}`}>2.1 Anatomi Buah Kopi</strong> — Buah kopi secara botani disebut <em className={theme.accent}>coffee cherry</em>. 
              ...
              <div className={`p-8 rounded-2xl ${theme.highlight} border ${theme.border} shadow-lg`}>
                <h3 className={`font-bold text-2xl ${theme.textHeading} mb-5`}>2.2 Apa Itu Fermentasi?</h3>
                <p className={`${theme.text} text-lg leading-relaxed`}>
          ...
          {/* Bab 3 - Roast Level - Enhanced visual */}
          <motion.section 
            id="bab-3"
            data-chapter={3}
            initial="hidden"
          ...
          >
            <div className={`flex flex-col sm:flex-row sm:items-center gap-6 mb-12 pb-6 border-b-2 ${theme.border}`}>
              <span className={`text-7xl sm:text-8xl font-black ${theme.accent} opacity-20 leading-none`}>03</span>
              <div className="flex-1">
                <h2 className={`text-4xl sm:text-5xl font-bold ${theme.textHeading} mb-3`}>Roast Level & Flavor</h2>
                <p className={`text-lg ${theme.textMuted}`}>Maillard Reaction & Development</p>
              </div>
            </div>

            <div className="space-y-8">
              {/* Enhanced Roast Level Visual */}
              <div className={`p-8 rounded-2xl ${darkMode ? 'bg-neutral-900/60' : 'bg-stone-100'} border ${theme.border} shadow-lg`}>
                <h3 className={`font-bold text-2xl ${theme.textHeading} mb-8 text-center`}>Spektrum Roast Level</h3>
          ...
          >
            <div className={`flex flex-col sm:flex-row sm:items-center gap-6 mb-12 pb-6 border-b-2 ${theme.border}`}>
              <span className={`text-7xl sm:text-8xl font-black ${theme.accent} opacity-20 leading-none`}>04</span>
              <div className="flex-1">
                <h2 className={`text-4xl sm:text-5xl font-bold ${theme.textHeading} mb-3`}>Grind Size & Extraction</h2>
                <p className={`text-lg ${theme.textMuted}`}>Teori Ekstraksi dan Partikel</p>
              </div>
            </div>

            <div className="space-y-8">
              <div className={`p-8 rounded-2xl ${theme.highlight} border ${theme.border} shadow-lg`}>
                <h3 className={`font-bold text-2xl ${theme.textHeading} mb-5`}>4.1 Apa Itu Ekstraksi?</h3>
              ...
              <blockquote className={`pl-8 border-l-4 ${theme.accentBorder} ${theme.accent} font-semibold text-2xl leading-relaxed`}>
          ...
          >
            <div className={`flex flex-col sm:flex-row sm:items-center gap-6 mb-12 pb-6 border-b-2 ${theme.border}`}>
              <span className={`text-7xl sm:text-8xl font-black ${theme.accent} opacity-20 leading-none`}>05</span>
              <div className="flex-1">
                <h2 className={`text-4xl sm:text-5xl font-bold ${theme.textHeading} mb-3`}>Espresso Fundamentals</h2>
                <p className={`text-lg ${theme.textMuted}`}>Tekanan 9 Bar dan Parameter Kritis</p>
              </div>
            </div>

            <div className="space-y-8">
              <div className={`p-8 rounded-2xl ${theme.highlight} border ${theme.border} shadow-lg`}>
                <h3 className={`font-bold text-2xl ${theme.textHeading} mb-6 text-center`}>5.1 Definisi Ilmiah Espresso</h3>
                <p className={`${theme.text} text-xl leading-relaxed mb-8 text-center max-w-3xl mx-auto`}>
          ...
          <motion.section 
            ...
            className="mb-28"
          >
            <div className={`p-12 rounded-3xl ${theme.highlight} border ${theme.border} text-center shadow-2xl`}>
              <motion.div 
              ...
              <h3 className={`text-3xl sm:text-4xl font-bold ${theme.textHeading} mb-4`}>15 Bab Lainnya</h3>
          ...
          {/* Sample content for remaining chapters - Enhanced */}
          {[
            ...
            >
              <div className={`flex flex-col sm:flex-row sm:items-center gap-6 mb-12 pb-6 border-b-2 ${theme.border}`}>
                <span className={`text-7xl sm:text-8xl font-black ${theme.accent} opacity-20 leading-none`}>
                  {String(chapter.id).padStart(2, '0')}
                </span>
                <div className="flex-1">
                  <h2 className={`text-4xl sm:text-5xl font-bold ${theme.textHeading} mb-3`}>{chapter.title}</h2>
                  <p className={`text-lg ${theme.textMuted}`}>{chapter.subtitle}</p>
                </div>
              </div>
              
              <div className={`p-8 rounded-2xl ${theme.highlight} border ${theme.border} shadow-lg`}>
                <p className={`${theme.text} text-xl leading-relaxed`}>{chapter.content}</p>
              </div>
              ...
              {chapter.id === 20 && (
                <motion.div 
                  ...
                  viewport={{ once: true }}
                  className={`mt-10 p-10 rounded-3xl ${theme.accentBg} border-2 ${theme.accentBorder} text-center shadow-2xl`}
                >
                  <p className={`text-2xl sm:text-3xl font-light italic ${theme.accent} mb-6 leading-relaxed`}>
          ...
          <motion.footer 
            ...
            viewport={{ once: true }}
            className={`pt-16 border-t-2 ${theme.border} text-center`}
          >
            ...
            <p className={`text-xl font-semibold ${theme.textHeading} mb-3`}>
      ...
      <AnimatePresence>
        {showScrollTop && (
          <motion.button
            ...
            whileTap={{ scale: 0.9 }}
            onClick={scrollToTop}
            className={`fixed bottom-8 right-8 p-4 rounded-full shadow-2xl z-40 ${theme.accentBg} ${theme.accent} border-2 ${theme.accentBorder} backdrop-blur-lg`}
          >
            <ArrowUp size={28} />
          </motion.button>
        )}
